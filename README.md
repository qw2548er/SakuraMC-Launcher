...
> ⚠️ **v0.1.0 是初代版本**, UI/页面框架已搭好, 但部分功能的**真实业务逻辑**还在开发中, 在 App 内会显示「🚧 未开发」横幅. 计划在后续版本逐个实现.

### ✅ 已完整实现

| 模块 | 状态 | 说明 |
|------|------|------|
| 🎨 玻璃态 UI + 樱花粉紫主题 | ✅ 完整 | 所有页面已实现 |
| 👤 离线账号系统 | ✅ 完整 | 添加/选择/删除, 自动生成 UUID |
| 📦 Minecraft 版本清单 | ✅ 完整 | 加载 Mojang 官方 manifest |
| 🧩 Mod 加载器信息展示 | ✅ 完整 | Forge/Fabric/Quilt/OptiFine 状态 |
| 🖥️ 服务器添加/删除/PING | ✅ 完整 | SLP 协议测速, MOTD/在线人数 |
| 🌸 樱花穿透账号登录 | ✅ 完整 | natfrp.com API 真实对接 |
| 🌸 节点列表与状态 | ✅ 完整 | 节点状态/带宽/延迟 |
| 🌸 隧道 CRUD | ✅ 完整 | 创建/编辑/删除/查询 |
| 🌸 流量查询 | ✅ 完整 | 实时流量统计, 进度条 |
| 🌸 frpc 配置文件生成 | ✅ 完整 | INI/TOML 双格式 |
| 🌸 frpc 二进制下载链接 | ✅ 完整 | Win/Linux/Mac/Android 全平台 |
| ⚙️ 启动命令生成 | ✅ 完整 | Java 命令 + .bat / .sh 脚本 |
| ⚙️ JVM 内存配置 | ✅ 完整 | 最小/最大堆内存设置 |
| ⚙️ 数据导入/导出 | ✅ 完整 | 一键备份/恢复所有配置 |

### 🚧 v0.1.0 未开发 (UI 已搭好, 真实逻辑后续实现)

| 功能 | 显示位置 | 计划版本 | 说明 |
|------|----------|----------|------|
| 游戏启动 | 首页「启动游戏」 | v0.2.0 | 需 PC 端 Java 运行时, 手机端用启动脚本方案 |
| 本地服务器启动 | 服务器页「启动」按钮 | v0.2.0 | 需 Java 进程执行 .jar |
| frpc 进程启动 | 穿透页「启动 frpc」 | v0.2.0 | 需 Node.js child_process 或原生插件 |
| 微软账号 OAuth | 账号页「微软登录」 | v0.2.0 | 需 Azure 应用 Client ID |
| 游戏版本下载 | 版本页 | v0.3.0 | 需 Mojang/BMCL API 完整下载逻辑 |
| Mod 下载与管理 | Mod 页 | v0.4.0 | 需 CurseForge/Modrinth API |
| 自动 Java 检测 | 设置页 | v0.2.0 | 需原生 FS 权限 |

> 💡 **为何先发初代?** UI 框架和 API 抽象层已经稳定, 先让用户体验完整界面, 后续版本只需填充真实业务逻辑, 不会破坏整体架构.

### 🎮 启动器核心
- ✅ **多账号系统** - 微软 OAuth (设备码流程) + 离线账号
- ✅ **皮肤预览** - 集成 mc-heads.net 头像/皮肤/披风
- ✅ **版本管理** - Mojang 官方 / BMCL 镜像 / MCBBS 镜像, 支持 1.7~1.21+ 全部版本
- 🚧 **下载隔离** - 多版本并存, 独立游戏目录 (UI 完成, 下载逻辑待 v0.3.0)
...
- ✅ **OptiFine** - 性能优化 Mod
- 🚧 **Mod 一键安装** - UI 已完成, 真实下载逻辑待 v0.4.0

### 🖥️ 服务器管理
- 🚧 **本地服务器** - 一键启动, 状态监控 (UI 完成, Java 进程启动待 v0.2.0)
...
- ✅ **启动命令** - 直接复制粘贴运行
- 🚧 **App 内启动 frpc** - H5 端通过下载配置 + 启动脚本方式实现, 原生 App 一键启动待 v0.2.0
...
- ✅ **数据导出** - 一键备份全部配置
- ✅ **清空数据** - 一键重置
- 🚧 **自动 Java 检测** - 浏览器内不可用, 待 v0.2.0 (原生 App)
