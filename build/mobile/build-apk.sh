...
if [ -f "$APK_PATH" ]; then
    ...
    mkdir -p "$OUTPUT_DIR"
    cp "$APK_PATH" "$OUTPUT_DIR/SakuraMC-Launcher-v0.5.3-android.apk"

    echo ""
    echo "✅ 构建成功!"
    echo "📂 APK 路径: $OUTPUT_DIR/SakuraMC-Launcher-v0.5.3-android.apk"
    echo "📦 文件大小: $(du -h "$OUTPUT_DIR/SakuraMC-Launcher-v0.5.3-android.apk" | cut -f1)"
else
